/**
 * Run to print ACTUAL Vectoris marketing_background provider prompt:
 *   PRINT_VECTORIS_PROMPT=1 npx vitest run src/ligs/image/__tests__/printVectorisPrompt.test.ts
 *
...
const vectorisProfile = parseVoiceProfile({
  id: "vp_studio",
  version: "1.0.0",
  ...
  ligs: {
    primary_archetype: "Vectoris",
    secondary_archetype: null,
    blend_weights: {},
  },
  descriptors: ["directional", "momentum", "path", "forward"],
  cadence: {
...
describe("print Vectoris marketing_background prompt (run with PRINT_VECTORIS_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_VECTORIS_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(vectorisProfile, {
    ...
    console.log(`\n========== VECTORIS marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
