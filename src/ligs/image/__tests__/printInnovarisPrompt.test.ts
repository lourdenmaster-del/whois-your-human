/**
 * Run to print ACTUAL Innovaris marketing_background provider prompt:
 *   PRINT_INNOVARIS_PROMPT=1 npx vitest run src/ligs/image/__tests__/printInnovarisPrompt.test.ts
 *
...
const innovarisProfile = parseVoiceProfile({
  id: "vp_studio",
  version: "1.0.0",
  ...
  ligs: {
    primary_archetype: "Innovaris",
    secondary_archetype: null,
    blend_weights: {},
  },
  descriptors: ["innovate", "fresh", "breakthrough", "reimagine"],
  cadence: {
...
describe("print Innovaris marketing_background prompt (run with PRINT_INNOVARIS_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_INNOVARIS_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(innovarisProfile, {
    ...
    console.log(`\n========== INNOVARIS marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
