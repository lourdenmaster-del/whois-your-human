/**
 * Run to print ACTUAL Duplicaris marketing_background provider prompt:
 *   PRINT_DUPLICARIS_PROMPT=1 npx vitest run src/ligs/image/__tests__/printDuplicarisPrompt.test.ts
 *
...
const duplicarisProfile = parseVoiceProfile({
  id: "vp_studio",
  version: "1.0.0",
  ...
  ligs: {
    primary_archetype: "Duplicaris",
    secondary_archetype: null,
    blend_weights: {},
  },
  descriptors: ["mirror", "reflect", "pair", "balance"],
  cadence: {
...
describe("print Duplicaris marketing_background prompt (run with PRINT_DUPLICARIS_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_DUPLICARIS_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(duplicarisProfile, {
    ...
    console.log(`\n========== DUPLICARIS marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
