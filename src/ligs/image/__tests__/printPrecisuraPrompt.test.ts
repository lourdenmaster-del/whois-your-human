/**
 * Run to print ACTUAL Precisura marketing_background provider prompt:
 *   PRINT_PRECISURA_PROMPT=1 npx vitest run src/ligs/image/__tests__/printPrecisuraPrompt.test.ts
 *
...
const precisuraProfile = parseVoiceProfile({
  id: "vp_studio",
  version: "1.0.0",
  ...
  ligs: {
    primary_archetype: "Precisura",
    secondary_archetype: null,
    blend_weights: {},
  },
  descriptors: ["precise", "exact", "measured", "clear"],
  cadence: {
...
describe("print Precisura marketing_background prompt (run with PRINT_PRECISURA_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_PRECISURA_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(precisuraProfile, {
    ...
    console.log(`\n========== PRECISURA marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
