/**
 * Run to print ACTUAL Tenebris marketing_background provider prompt:
 *   PRINT_TENEBRIS_PROMPT=1 npx vitest run src/ligs/image/__tests__/printTenebrisPrompt.test.ts
 *
...
const tenebrisProfile = parseVoiceProfile({
  id: "vp_studio",
  version: "1.0.0",
  ...
  ligs: {
    primary_archetype: "Tenebris",
    secondary_archetype: null,
    blend_weights: {},
  },
  descriptors: ["shadow", "depth", "mystery", "quiet"],
  cadence: {
...
describe("print Tenebris marketing_background prompt (run with PRINT_TENEBRIS_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_TENEBRIS_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(tenebrisProfile, {
    ...
    console.log(`\n========== TENEBRIS marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
