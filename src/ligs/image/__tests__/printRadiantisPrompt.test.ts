/**
 * Run to print ACTUAL Radiantis marketing_background provider prompt:
 *   PRINT_RADIANTIS_PROMPT=1 npx vitest run src/ligs/image/__tests__/printRadiantisPrompt.test.ts
 *
...
const radiantisProfile = parseVoiceProfile({
  id: "vp_studio",
  version: "1.0.0",
  ...
  descriptors: ["luminous", "clarity", "radiance", "uplifting"],
  cadence: {
...
describe("print Radiantis marketing_background prompt (run with PRINT_RADIANTIS_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_RADIANTIS_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(radiantisProfile, {
    ...
    console.log(`\n========== RADIANTIS marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
