/**
 * Run to print ACTUAL Obscurion marketing_background provider prompt:
 *   PRINT_OBSCURION_PROMPT=1 npx vitest run src/ligs/image/__tests__/printObscurionPrompt.test.ts
 *
...
const obscurionProfile = parseVoiceProfile({
  id: "vp_studio",
  version: "1.0.0",
  ...
  ligs: {
    primary_archetype: "Obscurion",
    secondary_archetype: null,
    blend_weights: {},
  },
  descriptors: ["obscure", "layered", "enigmatic", "depth"],
  cadence: {
...
describe("print Obscurion marketing_background prompt (run with PRINT_OBSCURION_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_OBSCURION_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(obscurionProfile, {
    ...
    console.log(`\n========== OBSCURION marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
