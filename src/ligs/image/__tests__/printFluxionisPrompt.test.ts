/**
 * Run to print ACTUAL Fluxionis marketing_background provider prompt:
 *   PRINT_FLUXIONIS_PROMPT=1 npx vitest run src/ligs/image/__tests__/printFluxionisPrompt.test.ts
 *
...
const fluxionisProfile = parseVoiceProfile({
  id: "vp_studio",
  version: "1.0.0",
...
describe("print Fluxionis marketing_background prompt (run with PRINT_FLUXIONIS_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_FLUXIONIS_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(fluxionisProfile, {
    ...
    console.log(`\n========== FLUXIONIS marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
