/**
 * Run to print ACTUAL Structoris marketing_background provider prompt:
 *   PRINT_STRUCTORIS_PROMPT=1 npx vitest run src/ligs/image/__tests__/printStructorisPrompt.test.ts
 *
...
const structorisProfile = parseVoiceProfile({
  id: "vp_studio",
  version: "1.0.0",
  ...
  ligs: {
    primary_archetype: "Structoris",
    secondary_archetype: null,
    blend_weights: {},
  },
  descriptors: ["structure", "order", "architecture", "logic"],
  cadence: {
...
describe("print Structoris marketing_background prompt (run with PRINT_STRUCTORIS_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_STRUCTORIS_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(structorisProfile, {
    ...
    console.log(`\n========== STRUCTORIS marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
