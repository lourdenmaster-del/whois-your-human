/**
 * Run to print ACTUAL Stabiliora marketing_background provider prompt:
 *   PRINT_STABILiora_PROMPT=1 npx vitest run src/ligs/image/__tests__/printStabilioraPrompt.test.ts
 *
...
const stabilioraProfile = parseVoiceProfile({
  ...
  descriptors: ["balance", "steady", "coherent", "regulated"],
  cadence: {
...
describe("print Stabiliora marketing_background prompt (run with PRINT_STABILiora_PROMPT=1)", () => {
  it("generates prompt and prints when env var set", () => {
    if (process.env.PRINT_STABILiora_PROMPT !== "1") return;

    const specBg = buildImagePromptSpec(stabilioraProfile, {
    ...
    console.log(`\n========== STABILiora marketing_background — ACTUAL SYSTEM OUTPUT ==========\n`);
