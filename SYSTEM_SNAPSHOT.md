...
| `PreviewRevealSequence` | `app/beauty/view/PreviewRevealSequence.jsx` | **Exemplar only.** Profile-driven, top-loaded 5-phase terminal reveal: archetype static image → archetype expression → final artifact. When `profile.isLockedPreview` (non-Ignis exemplars), phases 3 and 5 show pill-shaped "Unlocking" blur overlay. Replaces TerminalResolutionSequence for exemplar when profile is ready. |
