...
| `ArchetypeResolveCarousel` | `components/ArchetypeResolveCarousel.jsx` | Reusable archetype image carousel: cycles through archetype visuals, then resolves onto `finalArchetype`. Uses `lib/archetype-static-images`. Deceleration timing: `cycleDurationStartMs` 380 → `cycleDurationEndMs` 580. Props: `finalArchetype`, `onResolve`, `config`. Used by PreviewRevealSequence Phase 2 and OriginTerminalIntake. |
| `OriginTerminalIntake` | `components/OriginTerminalIntake.jsx` | Canonical /origin terminal flow: boot → intake (name, date, time, place, email) → processing → **archetypeResolve** (ArchetypeResolveCarousel) → postResolve (waitlist/CTA) → completeAwaitingEnterRedirect. Archetype from `resolveArchetypeFromDate(birthDate)`. WAITLIST_ONLY: Enter redirects to exemplar-{archetype}. |
