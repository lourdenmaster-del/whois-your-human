 ...
 * Beauty flow uses /api/beauty/submit so deriveFromBirthData always runs server-side.
 */
...
const BEAUTY_SUBMIT_ENDPOINT = "/api/beauty/submit";
const BEAUTY_DRY_RUN_ENDPOINT = "/api/beauty/dry-run";
 ...
 */
/**
 * POST to /api/beauty/dry-run. Saves report + BeautyProfileV1 to Blob (when configured).
 * Use when dryRun is true so previews and /beauty/view work locally for $0.
 * @returns {Promise<{ reportId: string, beautyProfile: { report, image, emotionalSnippet, imageryPrompts } }>}
 */
export async function submitToBeautyDryRun(formData) {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
  try {
    const res = await fetch(BEAUTY_DRY_RUN_ENDPOINT, {
      method: "POST",
...
/**
 * POST to /api/beauty/submit. Runs server-side derivation (deriveFromBirthData) then
 ...
 */
export async function submitToBeautySubmit(formData, options = {}) {
  const controller = new AbortController();
  ...
  try {
    const res = await fetch(BEAUTY_SUBMIT_ENDPOINT, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
/**
 * POST to the E.V.E. API (LIGS engine + Beauty filter). Prefer submitToBeautySubmit
 * for the Beauty page so deriveFromBirthData always runs server-side.
 * @param {Object} formData - { name, birthDate, birthTime, birthLocation, email, notes? }
 * @returns {Promise<{ reportId?: string, vector_zero, light_signature, archetype, deviations, corrective_vector, imagery_prompts }>}
 */
