 ...
 * Body: WHOIS Human Registration Card — renderFreeWhoisCard(report). Same report/helpers as preview; full report (renderFreeWhoisReport) remains in code for reference.
 *
 * Sender verification:
...
export async function sendWaitlistConfirmation(
  ...
  const html = renderFreeWhoisCard(report, { siteUrl: SITE_URL });
  const text = renderFreeWhoisCardText(report, { siteUrl: SITE_URL });
