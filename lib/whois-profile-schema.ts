/**
 * WHOIS-owned profile schema. Canonical source for WhoisProfileV1.
 * Same shape as legacy BeautyProfileV1; preserved for backward compatibility.
 */

import type { VectorZero } from "@/lib/vector-zero";
import type { BeautyProfile } from "@/lib/eve-spec";
import type { WhoIsProtocol } from "@/lib/whois-protocol";

/** Stable engine/build identifier for audit. Preserved for persisted JSON compatibility. */
...
/**
 * Canonical WHOIS profile schema (v1) — matches the shape produced by /api/engine.
 * New saves include schemaVersion and engineVersion.
 */
export interface WhoisProfileV1 extends BeautyProfile {
  version: "1.0";
  ...
  /** FREE TIER: deterministic protocol from buildWhoisProtocol. Always present for protocol-created profiles. */
...
function hasRequiredWhoisProfileV1(json: unknown): json is WhoisProfileV1 {
...
/**
 * Asserts that json conforms to WhoisProfileV1. Throws if required fields are missing.
 * Preserves BEAUTY_PROFILE_SCHEMA_MISMATCH for backward compatibility.
 */
export function assertWhoisProfileV1(json: unknown): asserts json is WhoisProfileV1 {
  if (!hasRequiredWhoisProfileV1(json)) {
