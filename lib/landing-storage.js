...
/** Origin terminal intake: form data passed to /beauty/view for local preview resolution. */
const ORIGIN_INTAKE_KEY = "ligs_origin_intake";

export function saveOriginIntake(formData) {
  if (typeof window === "undefined") return;
  try {
    const payload = {
      name: formData?.name ?? "",
    ...
    window.localStorage.setItem(ORIGIN_INTAKE_KEY, JSON.stringify(payload));
  } catch (e) {
    console.warn("Failed to save origin intake to localStorage", e);
  }
}

export function getOriginIntake() {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(ORIGIN_INTAKE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
...
export function clearOriginIntake() {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.removeItem(ORIGIN_INTAKE_KEY);
  } catch (e) {
    console.warn("Failed to clear origin intake from localStorage", e);
  }
}
...
/** WHOIS-owned unlock key. Canonical flow uses this. */
const WHOIS_UNLOCK_KEY = "ligs_whois_unlocked";
...
/**
 * WHOIS-owned unlock: set after checkout success.
 * Writes to WHOIS key only; isWhoisUnlocked reads both keys for legacy compatibility.
 */
export function setWhoisUnlocked() {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(WHOIS_UNLOCK_KEY, "1");
  } catch (e) {
    console.warn("Failed to set whois unlocked", e);
  }
}

/**
 * WHOIS-owned unlock check. Returns true if user has completed checkout.
 * Reads both WHOIS and legacy beauty keys so existing unlocked users remain recognized.
 */
export function isWhoisUnlocked() {
  if (typeof window === "undefined") return false;
  try {
    return (
      window.localStorage.getItem(WHOIS_UNLOCK_KEY) === "1" ||
      window.localStorage.getItem(BEAUTY_UNLOCK_KEY) === "1"
    );
  } catch (e) {
    return false;
  }
